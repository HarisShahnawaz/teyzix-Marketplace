...
const Home = () => {
  ...
  return (
    <motion.div 
      ...
      className="bg-slate-50/50 dark:bg-slate-950 min-h-screen font-sans transition-all duration-300"
    >
      
      {/* 🚀 Dynamic Hero Section */}
      <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/80 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight leading-tight">
            ...
            {!user && (
              <div className="flex items-center space-x-4 pt-2">
                ...
                <Link to="/login" className="text-slate-700 dark:text-slate-300 hover:text-[#1dbf73] font-bold px-4 py-2 transition-all">
                  Sign In
                </Link>
              </div>
            )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-xl border border-dashed border-slate-200 dark:border-slate-800 shadow-xs">
            <p className="text-sm text-slate-500 dark:text-slate-400">No professional services have been listed yet.</p>
          </div>
        ) : (
            ...
            {services.map((service) => {
              ...
              return (
                <motion.div
                  key={service._id}
                  whileHover={{ y: -4 }}
                  className="bg-white dark:bg-slate-900 rounded-xl shadow-xs hover:shadow-md border border-slate-200/60 dark:border-slate-800/60 flex flex-col justify-between overflow-hidden transition-all duration-200 group h-full"
                >
                  <div>
                    {/* 📸 Clean Thumbnail Image Box (No tags on top!) */}
                    <Link to={user ? `/services/${service._id}` : '/login'} className="block relative aspect-video w-full overflow-hidden bg-slate-50 dark:bg-slate-800">
                      <img 
                        src={cardBanner} 
                        alt={service.title} 
                    ...
                    <div className="p-4 space-y-2.5">
                      ...
                      <div className="space-y-1">
                        <Link to={user ? `/services/${service._id}` : '/login'} className="block">
                          <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 hover:text-[#1dbf73] transition-colors line-clamp-1 capitalize tracking-tight">
                            {service.title}
                          </h3>
                        </Link>
                        ...
                        <div className="flex items-center gap-1 text-xs font-bold min-h-[16px]">
                          {service.averageRating && service.averageRating > 0 ? (
                            <>
                              ...
                              <span className="text-slate-700 dark:text-slate-300 text-xs">{Number(service.averageRating).toFixed(1)}</span>
                              <span className="text-slate-400 font-normal text-[10px]">({service.reviewCount || 0})</span>
                            </>
                          ) : (
                            <span className="text-slate-400 dark:text-slate-500 font-normal text-[11px] italic">No reviews</span>
                          )}
                        </div>

                        {/* Beautiful inline category badge instead of a giant tag on picture */}
                        {service.category && (
                          <span className="flex items-center gap-1 text-[10px] font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md capitalize max-w-[120px] truncate">
                  ...
                  {/* 💳 Price Footer Row */}
                  <div className="px-4 py-2.5 bg-slate-50/50 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                    <Link 
                    ...
                    <div className="text-right">
                      <span className="block text-[9px] text-slate-400 font-medium uppercase tracking-wider -mb-0.5">Starting At</span>
